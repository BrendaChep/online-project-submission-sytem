    ...
    <div class="w3-col m9">
      ...
      <div id="main">
        ...
        <div class="w3-row-padding">
          ...
          <div class=" "> 
           <div class="w3-container w3-padding">
             <p class="w3-small w3-right-align">Note assigned students will appear under the <button onclick="viewGroups()" class="w3-btn w3-blue w3-left-align"><i class="fa fa-users fa-fw "></i>View Students </button> button</p>    
             <div class="w3-center">
               <?php 
               $sqlgrp = mysqli_query($dbcon, "SELECT * FROM `approvedmembers` WHERE approval='approved'") or die(mysqli_error($dbcon));
           // $group_row = mysqli_fetch_array($sqlgrp);
               $num_row = mysqli_num_rows($sqlgrp);

               if ($num_row < 0) { 
                echo "There are students to assign";
            ...
            if (isset($_POST['assign'])) {
              ...
              $assignedsup = mysqli_query($dbcon, "INSERT INTO `stud`(`appId`,`approval`, `empId`) VALUES ('$randId','1','$assignedsupervisor')") or die(mysqli_error($dbcon));

              $gresult = mysqli_query($dbcon, "SELECT * FROM stud WHERE appId = '$randId'") or die(mysql_error());
              ...
              $approvesql = mysqli_query($dbcon, "UPDATE approvedmembers SET approval='assigned' WHERE sugId = '$assignId'") or die(mysqli_error($dbcon));
              if($approvesql) { 
